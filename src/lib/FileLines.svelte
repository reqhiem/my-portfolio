<script>
  ...
  function generateDots(file, svgWidth) {
    ...
    for (let r = 0; r < dotRows; r++) {
      ...
      const rowDots = rowLines
        .map(
          (line, idx) =>
            `<tspan class="dot" data-index="${r * maxDotsPerRow + idx}" style="fill:${colorScale(line.type)}">•</tspan>`
        )
        .join("");
  ...
  $: if (svg) {
    ...
    // Animate position transitions for the race effect
    groups
      .transition()
      .duration(function(d, i) {
        ...
        return distance * 2; // 2ms per pixel
      })
      .attr("transform", (d, i) => `translate(0, ${positions[i]})`);
    ...
    groups.each(function(d) {
      ...
      unitDotsSel.html(d => generateDots(d, svgWidth));
